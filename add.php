<?php 
    ...
    if($_SERVER['REQUEST_METHOD'] = "POST") {
        ...
        $query = mysqli_query($con, "SELECT * from diary.entries WHERE username='$username'");  // Retrieves all data belonging to the specified username

        // Check if user exists
        $exists = mysqli_num_rows($query);  // If it exits, the number that $exists will hold will be greater than 0

        if ($exists > 0) {
            mysqli_query($con, "UPDATE diary.entries SET entry = CONCAT(entry, '$entry') WHERE username='$user';");
        } else {
            mysqli_query($con, "INSERT INTO `diary`.`entries` (`username`, `entry`, `date`) VALUES ('$user', '$entry', '$date');");
        }
